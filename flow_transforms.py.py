...
class SelectFrames(object):
    ...
    def __call__(self, video_buf):
        frame_count=video_buf.shape[0]
        n_clips=frame_count//self.num_frames
        video_buf=video_buf[:n_clips*self.num_frames, :,:,:]
        sp=video_buf.shape
        return video_buf.reshape([n_clips, self.num_frames, sp[1], sp[2], sp[3]])

class SplitFramePairs(object):
    def __init__(self):
        pass
    def __call__(self, clip):
        shp=clip.shape
        split_clip=np.zeros([shp[i]-1 if i==0 else shp[i] for i in range(len(shp))])
        return np.stack([clip[:-1,:], clip[1:,:]], axis=1)
        # split_clip=np.zeros([shp[0], shp[1]-1, 2, shp[2], shp[3], shp[4]])
        # split_clip[:,:,0,:]=clip[:,:-1, :]
        # split_clip[:,:,1,:]=clip[:,1:,:]
...
#         assert (input.shape[-1]==len(self.mean)) and (input.shape[-1]==len(self.std)), "Mean are std are per channel"
